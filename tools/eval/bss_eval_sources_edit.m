function [SDR,SIR,SAR,s_target,e_interf,e_artif]=bss_eval_sources_edit(se,s,flen)
...
%
% [SDR,SIR,SAR,perm]=bss_eval_sources(se,s,flen)
%
% Inputs:
...
SIR=zeros(nsrc,1);
SAR=zeros(nsrc,1);
s_target    = zeros(nsrc, nsampl);
s_true      = zeros(nsrc, nsampl);
e_spat      = zeros(nsrc, nsampl);
e_interf    = zeros(nsrc, nsampl);
e_artif     = zeros(nsrc, nsampl);
for j=1:nsrc
    [s_true(j,:),e_spat(j,:),e_interf(j,:),e_artif(j,:)]=bss_decomp_mtifilt(se(j,:),s,j,flen);
    [SDR(j),SIR(j),SAR(j)]=bss_source_crit(s_true(j,:),e_spat(j,:),e_interf(j,:),e_artif(j,:));
end
s_target = s_true + e_spat;
...
e_spat=project(se,s(j,:,:),flen)-s_true; % s_target_j = s_true + e_spat (see below)
% Interference
